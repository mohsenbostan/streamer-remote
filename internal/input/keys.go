...
// Virtual-key codes for the subset of keys a streamer's viewers can trigger.
...
// left/right specific variants a viewer might type instead.
...
// IsKeyKnown reports whether name maps to a supported virtual-key code.
func IsKeyKnown(name string) bool {
	_, ok := vkCodes[name]
	return ok
}
